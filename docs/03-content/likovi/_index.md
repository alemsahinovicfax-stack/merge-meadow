---
status: draft
tags: [hub, likovi]
---

# Likovi

Cute pastel ljubimci — cosmetic + flavor, bez borbenih statova u v1.

## Popis (launch scope)

### Pip (bunny) — default

| Aspekt | Vrijednost |
|--------|------------|
| Unlock | Od starta |
| Uloga | Tutorial companion, lane run sprite |
| Bonus v1 | Cosmetic only |
| Vizual | Bijeli/zelenkasti, **flat cartoon** portrait |

### Mochi (cat) — unlock kamp Lv.2

| Aspekt | Vrijednost |
|--------|------------|
| Unlock | Kamp level 2 ili 20 T3 orbs |
| Uloga | Zamjena skin za run |
| Bonus v1 | Cosmetic only |

### Bramble (fox) — post-slice

| Aspekt | Vrijednost |
|--------|------------|
| Unlock | 50 runova completed |
| Uloga | Kolekcija |
| Launch | OUT v1 — dodati u update |

## Povezano

- [[../03-content/svijet-i-lore|svijet-i-lore]]
- [[../04-experience/art-direction|art-direction]]
