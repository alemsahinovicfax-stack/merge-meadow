...
- **Home polje sezone pass 2 — preneseno u igru** — iz `design_handoff_home_field_v2/`: odnos je obrnut. Livada je bila **606 od 1633 px (37 %)**, sada je **cijela stranica**, bez okvira i radiusa, a kontrole plutaju preko nje i zauzimaju ≈ 13 %. Trake su pomjerene na `[0.32, 0.68]` (nebo 0–523, daljina 523–1110, prednja 1110–1633) pa gornja grupa stoji cijela na nebu a donji red cijeli na prednjoj traci. Gore lijevo je **gift chest** (u polju je do sada bio sakriven — nova instanca iste `HomeGiftCard`, bez runtime reparenta) i **ispod njega korpa koja dijeli isti okvir 180 × 180**: lokot „Basket" · sjeme „Choose" + prsten pažnje · portret + **„+5 %"** u mint pilulici, pa je bonus stalno na ekranu. Gore desno je **jedna plocica za nadogradnje** (znak ︽ + dvije trake nivoa + zlatna tačka kad se može kupiti) umjesto dvije kartice koje su trošile 400 px; Magnet i Loot sele u bottom sheet 922, korpa u sheet 1326 — oba krem, isti obrazac. Donji red je **`Seasons 236 × 124 · Play 432 × 140 · Endless 236 × 124`** (bilo 300 × 124 gore i 656 × 176 + 360 × 176 dolje), Play centriran na x 540 i ne pomjera se kad se Endless otključa. Kontrast je riješen jezikom naljepnica: svaka kontrola je neprozirna s rubom 3 px ink (≥ 6,14 : 1 na najgoroj sezoni), svijetli tekst nikad ne stoji na livadi, a sjena znači „pritisni me". 13 mjesta je preraspoređeno u 4 dubine (88 · 116 · 136 · kruna 168, sve 20–30 % veće), Pip hoda u pojasu 778 × 131 ispred svih cvjetova — ništa nije pod keepout zonama. Ime sezone je 56 px na nebu, tagline otpada; čip izraslih nosi ikonu cvijeta iz chrome v2. Nove skripte `field_basket_button.gd` i `field_upgrades_button.gd`, novi `FieldOverlay` i `UpgradesOverlay` u sceni; obrisani `FieldTopRow`, `BasketCard`, stalni `FieldUpgradeStack`, `meadow_frame()` i tamni jezik stranice. Novi `home_field_overlay_smoke` (rectovi, dodir ≥ 120, block_hub_swipe, keepout, oba sheeta, tačno jedan loop); ažurirani `season_meadow_smoke`, `home_basket_picker_smoke`, `season_home_smoke`; suite 55/55, GUT 33/33; [[../04-experience/design-drafts/home-field-v2-cd-brief|brief + odstupanja]] · [[../04-experience/design-drafts/home-field-v2-izvjestaj|izvještaj]]

- **Brief: Home polje sezone pass 2 (Claude Design)** — livada prestaje biti prozor i postaje **stranica**: danas od 1633 px dobija samo **606 (37 %)**, a ostalih 980 troše četiri naslagana bloka (red „Seasons + ime sezone" 124, korpa 180, dvije kartice nadogradnji 192 + 192, Play red 180), pa Pip od 190 px hoda po traci visokoj nekoliko svojih visina. Od sada sve kontrole **plutaju preko livade**: gore lijevo **gift chest** (danas je u polju sakriven, vidi se samo na biranju sezone), ispod njega **korpa istog dizajna** koja otvara izbor sjemena trenutne sezone s portretima (jedan tip ispod drugog, +5 % spawn), gore desno **jedan ulaz u Magnet i Loot Boost** umjesto dvije stalne kartice, a dolje red **`Seasons · Play · Endless`** znatno manji nego danas (Play 656 × 176, Endless 360 × 176, Seasons 300 × 124) — i u varijanti bez Endlessa, prije kraja tutoriala. Mehanika je fiksna (nivoi 0–4, magnet 40 + 48 × nivo, loot ×1.0–×2.0, cijena 2 × ★1 cvijet, 13 mjesta s pragovima 1 / 5 / 10, korpa zaključana do prvog mergea). Glavni rizik je kontrast: chrome se seli s tamne stranice `#2E4733` na osam svijetlih pastelnih livada (`#E6F2DB` … `#FFC79E`) i tri trake svjetline, pa brief traži jedno rješenje za sve i keepout zone s novom tablicom 13 mjesta. Isporuka: zip `design_handoff_home_field_v2/` (`design/`, `assets/`, `godot/home_field_v2_export.json` + `ui_home_field.gd` + `field_tree.txt`); [[../04-experience/design-drafts/home-field-v2-cd-brief|brief]]

- **Hub chrome pass 2 + ikone valuta — preneseno u igru** — iz `design_handoff_hub_chrome_v2/`: trake su **dusk plum `#2A2233`** umjesto zelene `#1A241E` (ΔE prema Camp/Arena/karuselu 22–32, bilo 7–18 — chrome sada uokviruje tamne livade umjesto da izgleda kao njihova nijansa), footer je **144 px** (bio 180) i nosi **samo ikone**, a dodir ostaje cijeli slot 216 × 141. Aktivan tab se razlikuje na četiri načina (peach ploča, ikona u boji vs krem linija, 72 vs 64 px, indikator 72 × 8), ime taba je ostalo kao `tooltip_text`. Chipovi valuta su jedan **udubljen tamni well `#1F1926`** s krem brojem — boju nosi ikona, pa zlatno-na-zlatnom više ne postoji; treći chip broji **cvijeće** (`get_garden_crystal_total()`), dijamant izlazi iz huba i ostaje samo u run HUD-u (nema potrošnju u igri). Nove ikone **coina, sjemena i cvijeta idu kroz cijelu igru** — isti fajl u headeru, Campu, Shopu, na Home kartici sezone i kao **pickup u runu**, gdje je zamijenio proceduralni krug iz `coin_visual.gd`; `icon_seed_light` i `arena/icon_crystal` su ispali s poziva. Δ od 36 px koje je footer oslobodio otišlo je stranicama (1597 → **1633**) bez ijedne promjene mjere kartice: Camp sekcija 1289 / 1585, Arena polje 1589, Home kartica sezone 1136 uz dock i Play red 36 px niže, Shop i Journal duži skrol. Nova dva smokea (`hub_chrome_icons_smoke`, `run_coin_texture_smoke`) i šest ažuriranih (`hub_chrome_smoke`, `camp_layout_smoke`, `camp_section_fixed_smoke`, `arena_redesign_smoke`, `season_home_smoke`, `meta_hub_flow_smoke`); [[../04-experience/design-drafts/hub-chrome-v2-cd-brief|brief + odstupanja]] · [[../04-experience/design-drafts/hub-chrome-v2-izvjestaj|izvještaj]]

## 2026-09-24

- **Brief: Hub chrome pass 2 + ikone valuta (Claude Design)** — druga runda chromea: iz footera van imena tabova (ostaju samo ikone), nove intuitivne ikone za svih 5 tabova kao jedna porodica (Home = kućica; Camp vs Home moraju biti razlučivi na 64 px), footer sa 180 na 132–150 px uz dodir ≥ 120, i nova **zajednička ne-zelena boja** headera i footera (danas `#1A241E` se stapa s Camp `#2E4733` i Arena `#293D2E`). Header zadržava 143 px i lanac širina, ali treći chip više ne broji dijamante nego **cvijeće** (`get_garden_crystal_total()`) — dijamant nema potrošnju u igri i ostaje samo u run HUD-u. Glavni dio su **nove ikone coina, sjemena i cvijeta za cijelu igru**, ne samo za chrome: isti fajl se koristi na 20+ mjesta od 32 px (cijena u Campu) do 96 px (pickup u runu), a coin ikona zamjenjuje i proceduralni novčić koji run danas crta sam (`coin_visual.gd`) — brief nosi punu mapu mjesta, veličina i podloga (uključujući zamku „zlatna ikona na zlatnom chipu"). Visina koju footer oslobodi ide stranicama, ne karticama: Δ se raspoređuje po `PAGE_H` konstantama (Camp, Shop, Journal, Arena, Home polje), a za Home biranje sezone (apsolutni raspored `ui_stage.gd`) CD mora reći tačno gdje. Isporuka: zip `design_handoff_hub_chrome_v2/` (`design/`, `assets/icons/` sa svih 8 SVG-ova, `godot/hub_chrome_v2_export.json` + `ui_chrome.gd` + `hub_tree.txt`); [[../04-experience/design-drafts/hub-chrome-v2-cd-brief|brief]]

- **Camp pass 2 — preneseno u igru** — iz `design_handoff_camp_v2/`: kartica sezone pala s 422 na **276 px** (bez „Next free season" i „Details ↗", Unlock je pilula 300 × 120 s jednom riječju i lokotom dok fali), a sekcija Seeds/Flowers je **prikovana 20 px ispod nje i uvijek iste visine** (1253, odnosno 1549 bez kartice) — rupa koja je rasla s 215 na 375 px više ne postoji jer je `StackGap` obrisan. Red tabova nosi samo Seeds | Flowers (bez „Merge" prečice i podnaslova „Arena fuel" / „reward"), ime lijevo i broj tipova uz desni rub. Crtež u karticama je porastao ~45 % (okvir 104 → 128, crtež 66 → 96 / 100) uz istu karticu 489 × 176; rezervisani chip je sada isto 176 (badge „Kept · N / M" stoji u redu pipsa). Trade traka je bez „★☆☆ · 1 coin each" i „hold 10 / s", dugme je 300 × 120 s jednom riječju i ikonom (lokot / hold-stop), a držanje se sada **vidi**: fill je prodani dio gomile, svaki tik pošalje novčić 44 px do coin chipa (max 3) i chip dobije prsten, dok jedan tap kratko napuni i isprazni traku. Prazno stanje je bez rečenice objašnjenja, s manjim CTA (120 dodir / 100 vizuelno). Mehanika netaknuta (tap = 1, držanje = 10/s, cijene, rezervisano cvijeće, 500 + 20). Novi `camp_section_fixed_smoke`; ažurirani `camp_layout_smoke`, `camp_season_link_smoke`, `camp_trade_select_smoke`, `camp_hold_floor_smoke`, `camp_trade_hold_smoke`, `camp_crystal_select_smoke`, `camp_smoke_util`; suite 52/52, GUT 33/33; [[../04-experience/design-drafts/camp-v2-cd-brief|brief + odstupanja]] · [[../04-experience/design-drafts/camp-v2-izvjestaj|izvještaj]]

- **Home v2: dvije izvedbe, ostaje desktop** — PR #2 („Home season select v2 stage", Cursor agent) donio je drugu izvedbu istog ekrana (`game/themes/home_season/*.tres`, `home_season_token.gd`, `home_season_styles.gd`, uz zadržan `ui_home.gd` i `home_unlock_burst.gd`). Mergean je strategijom `ours` — ostaje desktop izvedba jer je ona u igri, provjerena smokeovima i screenshotovima, a Journal, Shop i Arena su građeni na `ui_stage.gd` tokenima iz nje. Varijanta iz PR-a nije u stablu; `git log` je čuva ako ikad zatreba.

- **Brief: Camp pass 2 (Claude Design)** — druga runda čišćenja Campa: gore van „Next free season" i „Details ↗" (tap na karticu i dalje vodi na Home), Unlock ostaje samo riječ „Unlock" i prestaje biti 988 × 132 px; sekcija Seeds/Flowers se prikiva uz karticu sezone i dobija fiksnu visinu (danas pluta pa rupa u sredini ide 215–375 px); iz reda tabova van „Merge" prečica, iz tabova podnaslovi „Arena fuel" / „reward"; art sjemena i cvijeta raste unutar iste kartice 489 × 176 (danas okvir 104, crtež 66/70); iz Trade bara van „★☆☆ · 1 coin each" i „hold 10 / s", dugme manje (danas 430 × 120), isto i u praznom stanju oba taba. Mehanika ostaje fiksna (tap = 1, držanje = 10/s, cijene po rijetkosti, rezervisano cvijeće, 500 + 20), a držanje mora ostati otkrivljivo bez rečenice. Isporuka: zip `design_handoff_camp_v2/` (`design/`, `assets/`, `godot/camp_v2_export.json` + `ui_camp.gd` + `camp_tree.txt`); [[../04-experience/design-drafts/camp-v2-cd-brief|brief]]

- **Arena — prostor umjesto HUD-a** — uklonjeni `ArenaHud` red (dnevni zadatak + stash), `HintLine` traka (13 poruka), `ComboMeter` pilula i `Done` dugme; ostaje samo polje. Playfield raste sa 1133 na **1553 px** (44 px pojasa dolje ostaje NavLockPillu), gnijezdo munchera ide sa 310 na 108 px od vrha stranice, a vreća i Pip 172 px niže; spawn dobija cijelu visinu (combo keepout otpao, `GRID_DENSE_ABOVE` 24 → 32). **Mehanika je netaknuta**: dnevni zadatak, garden stash, combo + coini, pour lockovi i leftover vacuum rade kao prije. Bez Done dugmeta sesija se završava sama — nav lock stoji dok ima sjemenki na polju, prvi tap na vreću otvara sesiju (`_start_session_if_needed`, čisti pour lockove), a `_end_session_if_settled` je zatvara kad polje ostane prazno (sve spojeno pa ostatak vraćen u vreću, ili je muncher pojeo sve); „Back to Camp" u „You need more seeds" overlayu je jedini ručni izlaz. Feedback bez HUD-a: Pip poskok, T3 kristal odleti u gornji desni ugao i nestane, combo nagrada javlja „+N" pop kod coin chipa (`meta_hub_controller.show_coin_earn_pop`), odbijen tap na vreću = punch vreće umjesto teksta. Merge Hint booster (IAP) svoju poruku sada dobija u oblačiću iznad vreće (`arena_cue.gd`, zamijenio `arena_hud.gd`). Novi `arena_session_end_smoke`, prepisani `arena_redesign_smoke` i `arena_nav_lock_smoke`, očišćeni `arena_combo_smoke` / `arena_daily_smoke`; suite 51/51; [[../04-experience/design-drafts/merge-arena-cd-brief|brief § Izmjena]]

- **Shop redizajn (Claude Design)** — iz `design_handoff_shop/`: pozadina `#2E4733`, jedan skrol sa 4 sekcije (Looks · Seasons · Boosters · Support) i sticky red chipova sa scroll-spyjem. Kozmetika je grupisana po slotu i ima pregled „Now | With it“ (staza i Pip iz `UiRun` × modulate, stranica Albuma sa zlatnim ramom); kupovina za coine ide u dva tapa („Buy & wear“ → „Spend N coins?“ → „Buy for N“), kupljeno se odmah nosi, a −N leti ispod coin chipa (`meta_hub_controller.show_coin_spend_pop`). Poruke stoje na kartici (ok 2,4 s, fail do sljedećeg tapa, pending žut) umjesto jedne `StatusLabel` linije. Season kartice su u mood boji s rosterom 3 × 2 i taglineom: „Get …“, „Play it on Home ↗“ kad je kupljena, Ember Fen „COMING SOON“ bez cijene. Boosteri: zaliha, „Use one“, „Go to Arena ↗“ kad je hint aktivan, „Bag is full“ na punom bagu. Support: Remove Ads, Starter Pack sa sadržajem u 3 chipa, Restore (samo van stub moda) i fair note. Jedna IAP kupovina u isto vrijeme (pulsirajuće „Waiting for store…“, ostalo 50 %). Novi `ui_shop.gd`, `ui_shop_buttons.gd`, `shop_cosmetic_preview.gd`, `shop_cosmetic_card.gd`, `shop_purchase_status.gd`, `shop_price_tag.gd`, `shop_jump_chip.gd`; `PipDraw.draw_pip` dobio `palette_override`. Novi `shop_booster_guard_smoke`, prepisani `shop_open_smoke` i `shop_cosmetic_buy_smoke`; **popravljen `shop_nav_smoke`** koji je vješao suite (statička `GameState` referenca u `--script`); [[../04-experience/design-drafts/shop-cd-brief|brief + odstupanja]] · [[../04-experience/design-drafts/shop-izvjestaj|izvještaj]]

## 2026-09-23

- **Brief: Shop (Claude Design)** — pravila iz koda (5 kozmetika 120–250 coina po slotovima, 2 boostera €0.99, 4 season packa €2.99–€3.49, Remove Ads €3.99, Starter Pack €1.99, Restore samo van stub moda), današnji raspored i 12 problema (jedini neredizajnirani ekran, kozmetika se kupuje naslijepo, tekst 16–28 px, feedback u jednoj liniji, dijamanti bez potrošnje, Ember Fen ćorsokak). Format kao za polje sezone: sloboda, **jedan dizajn**, zip paket `design_handoff_shop/` (`design/`, `assets/`, `godot/shop_export.json` + `ui_shop.gd` + `shop_tree.txt`); [[../04-experience/design-drafts/shop-cd-brief|brief]]

- **Journal / Bloom Album (1a)** — iz `design_handoff_journal/`: krem stranica, naslov „Bloom Album" u ink, red 1032×200 (ime + ★ + caption, tri slota 112: prazan prsten / bloom well / T3 gold kvadrat, NEW pilula gore lijevo + halo). Sezonska poglavlja s „N / 6 kept" i lokotom ako sezona nije otvorena. NEW ostaje vidljiv cijelu posjetu (ranije je `mark_collection_journal_viewed` gasio bedž u istom frameu); tab badge se i dalje briše na otvaranje, a odlazak sa stranice gasi NEW. Auto-scroll na prvi NEW. Golden Album frame + plaketa kad je `journal_gold` opremljen. `journal_new_snapshot_smoke` + `collection_journal_smoke` OK; paket `design_handoff_journal/` je u repou; [[../04-experience/design-drafts/journal-cd-brief|brief + odstupanja]] · [[../04-experience/design-drafts/journal-izvjestaj|izvještaj]]

- **HOME-20: Home Season Stage v2 — prepravljeno 1:1 po dizajnu** — prvi prenos se izlivao van ekrana (Home kolona 1286 px: stari Daily gift je imao min. širinu 438 px). Sada je raspored iz `design_handoff_home_v2/design/SeasonStage.dc.html` apsolutan: kartica 1032×1100 na (24, 24), dock 1080×222 na (0, 1148), Play 836×180 + Gift 180 na (24, 1394), toast na 250. Nova `HomeSeasonCard` crta SeasonArt (pruge, placeholder labela, lokot / prsten 520, bedž, strelice 96/132) → naslov 170 → roster (3 × 236 ili 6 × 214) → akcija po stanju (Open meadow · UnlockPoster s barovima · PriceTag + Buy · Coming soon). Dock (`SeasonBrowser` + `HomeDockToken`): broj / lokot + bar / dijamant / Soon / ✓, ▶ tačka, podizanje fokusa 120 ms, shake 180 ms + crveni obrub. Swipe na kartici s pregledom susjedne, klizanje 1080 px 280 ms, rubber-band 40 px; tokeni reaguju na otpuštanje, drag ide hubu. Unlock trenutak 420 ms (prsten, barovi se prazne 200 ms, boja lock → mood), toastovi "… unlocked · now playing", "Unlocked in Camp · now playing", "… is yours · now playing", "Unlock … first". Tutorial hint (oblačić + pulsirajući prsten oko Play). Nunito variable (OFL, `game/assets/fonts/nunito/`) samo za Home biranje; header/footer ostaju nepromijenjeni hub chrome. Boje prate HTML (mix sa zaokruživanjem, WCAG luminansa; poluprozirni rubovi se računaju kao neprozirni). Pikselna razlika dizajn vs igra u sadržaju 2–3 % (anti-aliasing teksta). Svjesna odstupanja: Country Bloom roster koristi pravi art igre umjesto `ph_*` placeholdera; otključana a neaktivna sezona dobija "PREVIEW · UNLOCKED / OWNED" (u mocku nije nacrtana); cijena je store string (€2.99); prije prve runde Gift stoji bez tačke. Obrisani v1 `ui_home.gd`, `home_progress_indicator.gd`, `home_premium_header.gd`, `home_bar.gd`, `home_unlock_burst.gd` i TopRow. Usput (HOME-19 polje): redoslijed livada → korpa → nadogradnje → Play (runtime `move_child` ga je kvario), jedan ▶ na Play, čitljiv "‹ Seasons". `season_home_smoke` prepisan (tačni rectovi, ništa van stranice, stanja, swipe, unlock, kupovina, Camp dolazak, poklon, hint); 47/48 smokeova OK (`shop_nav_smoke` pada kao i prije — statička referenca na `GameState` u `--script`). greske-katalog #17–#19

## 2026-09-22

- **Fail loot coini** — `finish_run` na failu koristi `ceil(50%)` kao `_halve_seed_bag`, da 1 coin ne padne na 0 (`round(0.5)==0` na desktopu).

- **HOME-19: Home — polje sezone u Godotu** — iz `design_handoff_home_field/`: livada 1032×605 na `#2E4733` s 13 stash-gated mjesta (1/5/10) i Pip 190; Seasons 300×124 uvijek dok je polje otvoreno; korpa 1032×180 + Choose; Magnet / Loot Boost kartice 192 px; Play 656×176 / Endless 360×176; picker bottom sheet 1080×1308, red 148; čvorovi ostaju u `HomeColumn` (bez runtime reparenta — CAMP-04 zamka); `season_meadow_smoke` / `home_basket_picker_smoke` / `season_home_smoke` OK
