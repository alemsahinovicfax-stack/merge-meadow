...
## Generisano iz design_handoff_home_field/godot/field_export.json.
## Nadograđuje UiHome (design_handoff_home/godot/ui_home.gd) — ovdje su samo NOVE
## ili promijenjene vrijednosti. Sve mjere su u px baze 1080 x 1920 i prenose se 1:1.

# ── Preuzeto iz UiHome (ponovljeno da fajl radi i sam) ───────────────────────
...
## Chrome ulazi iza livade: 5 blokova, 60 ms razmaka.
static func tween_chrome_in(blocks: Array) -> void:
	for i in blocks.size():
		var c: Control = blocks[i]
		c.modulate.a = 0.0
		var from_y := c.position.y + 40.0
		c.position.y = from_y
		var t := c.create_tween().set_parallel(true)
		t.tween_interval(ANIM.chrome_stagger * i)
		t.chain().tween_property(c, "position:y", from_y - 40.0, ANIM.chrome_in).set_ease(Tween.EASE_OUT)
